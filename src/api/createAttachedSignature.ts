    ...
    return eval(
      _generateCadesFn(function createAttachedSignature(): string {
        ...
        try {
          ...
          cadesSigner.TSAAddress = "http://cryptopro.ru/tsp/";
        } catch (error) {
          console.error(error);
        ...
        try {
          signature = __cadesAsyncToken__ + cadesSignedData.SignCades(cadesSigner, cadesplugin.CADESCOM_CADES_X_LONG_TYPE_1);
        } catch (error) {
          console.error(error);
